...
public final class GenerateGitIgnoreAction implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ev) {
        ...
        try {
            ...
            if (descriptor.getValue() == DialogDescriptor.OK_OPTION) {
                ...
                // create file
                if (!createFile(gitignoreContent)) {
                    // show dialog
    ...
    private boolean createFile(String gitignoreContent) throws IOException {
        FileObject projectDirectory = context.getProjectDirectory();
        File file = new File(FileUtil.toFile(projectDirectory), GITIGNORE_NAME);
        boolean isCreated = false;
        if (file.createNewFile()) {
            PrintWriter pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream(file), UTF8), true); // NOI18N
            try {
        ...
        // open file
        FileObject gitignore = FileUtil.toFileObject(file);
        if (gitignore != null) {
